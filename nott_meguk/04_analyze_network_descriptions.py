...
if __name__ == "__main__":
    ...
    # NOTE: The reference run should be adjusted accordingly.
    ...
    if order is not None:
        ...
        covs = covs[order] # shape: (n_states, n_channels, n_channels)
    ...
    save_path = os.path.join(DATA_DIR, "inference/psds.pkl")
    ...
    if os.path.exists(save_path):
        ...
        ts = load(os.path.join(BASE_DIR, f"data/{data_type}/trimmed_ts.pkl"))
    ...
    # Plot loss curves
    print("(Step 5-4) Plotting loss curves ...")
    history = load(os.path.join(DATA_DIR, "model/history.pkl"))
    DV.plot_loss(
        history,
        model_type=model_type,
        save_dir=os.path.join(FIG_DIR, "analysis"),
        epoch_step=(5 if model_type == "hmm" else 50)
    )

...
def find_on_off_indices(vector):
    """Finds the indices where a binary vector transitions
       from 0 to 1 (on) and from 1 to 0 (off).
    ...
    indices : list of tuples
        ...
        of consecutive 1s in the input vector.
    """
    # Handle empty input
    if not len(vector):
        return []
    
    # Get on and off indices
    vector = np.asarray(vector)  # ensure input is a numpy array
    vector_padded = np.concatenate(([0], vector, [0]))  # to detect sequences at edges
    diff = np.diff(vector_padded)

    on_indices = np.where(diff == 1)[0]
    off_indices = np.where(diff == -1)[0] - 1

    return list(zip(on_indices, off_indices))
